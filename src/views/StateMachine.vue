<template>
  <div class="my-1 mb-3 mx-auto text-center justify-content-center bg-light px-4 py-3" :class="{'rounded':!mobile}">
    <h3 class="text-center py-2 mb-3 font-weight-bold">Распознаватель слов в автоматах</h3>
    <div class="input-group mb-3">
      <input type="text" class="form-control" placeholder="слово для распознавания" v-model="input.text">
      <div class="input-group-append">
        <button class="btn btn-success" @click="analize()">Распознать</button>
      </div>
    </div>
    <div class="col text-light rounded text-center my-2 py-1" :class="{'bg-success':result=='распознал', 'bg-danger':result=='не распознал'}">{{result}}</div>
    <table class="table-responsive text-center align-middle my-auto mx-auto justify-content-center mt-2">
      ...
      <tr v-for="(s,index) in states" :key="index">
        ...
        <th class="border border-dark p-0" v-for="(t,index) in table[states.indexOf(s)]" :key="index">
          <input type="text" autocomplete="false" placeholder="следующее состояние" class="form-control border-0 text-center rounded-0" :class="{'font-weight-bold':t.nextState && t.nextState.length>0}" v-model="t.nextState" @input="t.nextState = t.nextState.toUpperCase()">
          <input type="text" autocomplete="false" placeholder="читать из стека" class="form-control text-center rounded-0" :class="{'border-top border-bottom border-left-0 border-right-0': input.pila, 'font-weight-bold':t.read && t.read.length>0}" v-model="t.read" v-if="input.pila">
          <input type="text" autocomplete="false" placeholder="записать в стек" class="form-control border-0 text-center rounded-0" :class="{'font-weight-bold':t.write && t.write.length>0}" v-model="t.write" v-if="input.pila">
          <input type="text" autocomplete="false" placeholder="заменить в слове" class="form-control border-0 text-center rounded-0" :class="{'border-top border-left-0 border-right-0': input.turing, 'font-weight-bold':t.write && t.write.length>0}" v-model="t.write" v-if="input.turing">
          <input type="number" autocomplete="false" placeholder="Инкрементация декрементация" class="form-control border-0 text-center rounded-0" :class="{'border-top border-left-0 border-right-0': input.turing, 'font-weight-bold':t.move}" v-model="t.move" v-if="input.turing">
        </th>
      </tr>
    </table>
    <div class="row col-12 mx-auto text-center my-3 text-center">
      <button class="btn col-lg col-12 btn-success my-2 mx-auto m-md-2" @click="addState">+1 Состояние</button>
      <button class="btn col-lg col-12 btn-success my-2 mx-auto m-md-2" @click="addTransition">+1 Переход</button>
    </div>
    <div class="col-12 my-2">
      <div class="custom-control custom-switch">
        <input type="checkbox" class="custom-control-input" id="pila" v-model="input.pila" @click="input.turing=false">
        <label class="custom-control-label" for="pila">Добавить стек</label>
      </div>  
    </div>
...
<i class="fa fa-file" aria-hidden="true"></i>
    ...
    <b-modal centered size="lg" title="История" hide-footer v-model="showHistory">
      <div class="input-group mb-3">
        <input type="text" class="form-control" v-model="input.tableName">
        <div class="input-group-append">
          <button class="btn btn-success" @click="addHistory()">Сохранить автомат</button>
        </div>
      </div>
      <div class="py-2 mx-auto text-center" :class="{'border-bottom':index!=history.length-1}" v-for="(table,index) in history" :key="index">
        ...
        <button class="btn btn-info d-inline-block mx-2" @click="tableHistoryShow = table" v-b-modal.showTable>См. таблицу и автомат</button>
        <button class="btn btn-success" @click="setActual(table); showHistory=false">Использовать</button>
      </div>
    </b-modal>
      ...
      <table class="table-responsive text-center align-middle justify-content-center m-2 p-2" v-if="tableHistoryShow">
        ...
        <tr v-for="(s,index) in tableHistoryShow.states" :key="index">
          ...
          <th class="border border-dark p-0" v-for="(t,index) in tableHistoryShow.table[tableHistoryShow.states.indexOf(s)]" :key="index">
            <p type="text" placeholder="следующее состояние" class="form-control border-0 text-center rounded-0 my-0" :class="{'font-weight-bold':t.nextState && t.nextState.length>0}">{{t.nextState}}</p>
            <p type="text" placeholder="читать из стека" class="form-control text-center rounded-0 my-0" :class="{'border-top border-bottom border-left-0 border-right-0': tableHistoryShow.pila, 'font-weight-bold':t.read && t.read.length>0}" v-if="tableHistoryShow.pila">{{t.read}}</p>
            <p type="text" :placeholder="{'записать в стек':tableHistoryShow.pila, 'тьюринг':tableHistoryShow.turing}" class="form-control border-0 text-center rounded-0 my-0" :class="{'border-top border-left-0 border-right-0': tableHistoryShow.turing, 'font-weight-bold':t.write && t.write.length>0}" v-if="tableHistoryShow.pila || tableHistoryShow.turing">{{t.write}}</p>
          </th>
        </tr>
      </table>
...
export default {
  ...
  data(){
    ...
    var test= {
      table: [[{ "nextState": "S1","state": "S0", "transition": "a"}, { "nextState": "S2","state": "S1", "transition": "b"}]],
      states: [{value:"S0",end:false} ,{value:"S1",end:false} ,{value:"S2",end:true}],
      transitions:  [{value:"a"} ,{value:"b"}],
      turing:false,
      pila:false,
      name: "test",
      text: "",
  ...
  methods:{
    ...
    deleteTransition(transition){
      if(confirm("Вы хотите удалить переход " + transition.value + "?")){
    ...
    analizePila(){
      ...
      return reconoce && estadoFinal && estadoFinal.end && pila.length==0?"распознал":"не распознал"
    },
    analizeTuring(){
      var col, lin, letra;
      ...
      return reconoce && palabra.join("") == this.input.resultTuring && estadoFinal && estadoFinal.end?"распознал":"не распознал";
    },
    analizeOther(){
      var palabra = this.input.text;
      var tabla = this.table;
      ...
      let estadoFinal = this.states[this.findIndex(this.states, estado)];
      return reconoce && estadoFinal && estadoFinal.end ?"распознал":"не распознал"
    },
    analize(){
      this.result="";
      if(this.table.length>0){
        if(this.input.text.length==0){
          this.result = this.transitions.find(el=>el.value=="") && this.states[0].end?"распознал":"не распознал";
